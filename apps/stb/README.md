...
Exact Store pin for this candidate branch: `bacbc9a08c6fa5dba3d7fc2b9fd0982646c3e5ed` (`GeorgePlattDemo/scan-to-build-store`, Store PR #9). Repin to the promoted Store main SHA before this System branch is merged.
...
Exact Store pin for this candidate branch: `bacbc9a08c6fa5dba3d7fc2b9fd0982646c3e5ed` (`GeorgePlattDemo/scan-to-build-store`, Store PR #9).
...
`ALCOVE_INSERT_V1` is the bounded Store boundary for Sarah's solid-board Alcove path. The project carries material class/species, project-derived 72/96 in parent demand, kept lengths, quantities, required operations, optional shelf-spot demand, hardware demand, and unresolved project conditions. It does not nominate Store board SKUs or calculate Store prices. Store returns the current species-specific SKUs, stock state, selling price, declared D-001 capability/refusal, calculation identity, and fresh evaluation receipt. Complete Alcove machine service and Q remain unresolved until Store owns a whole-board Alcove travel/economics standard. Shelf-elevation `FROM_BASE` face spotting currently fails closed because the Stage-2 spot envelope has not yet declared that location rule.
