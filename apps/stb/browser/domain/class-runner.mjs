...
import { planUser1XBraceDerivation } from '/domain/user1-xbrace-engine.mjs';
...
import { USER1_XBRACE_CONFIGURATION_KIND } from '/shared/user1-xbrace-rule.mjs';

export function planCandidateDerivation(input) {
  const classId = input.payload?.classReference?.classId ?? null;
  const configurationKind = input.payload?.configuration?.kind ?? null;
  if (classId === null && configurationKind === USER1_XBRACE_CONFIGURATION_KIND) {
    return planUser1XBraceDerivation(input);
  }
