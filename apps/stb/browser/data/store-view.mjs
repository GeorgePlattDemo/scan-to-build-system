...
function scopeForProjection(projection) {
  if (projection?.payload?.definitionKind === 'sheet.mode2.stencil.v1') {
    return STORE_SCOPES.SHEET_MODE2_STENCIL_V1;
  }
...
export async function loadStorePresentation(localRecordId, options = {}) {
  ...
  const applicability = await currentStoreAnswer(localRecordId, {
    ...
    scope: scopeForProjection(projection),
  });
...
export async function loadStoreHistory(localRecordId, currentCandidateRevisionId) {
  ...
  for (const request of [...requests].reverse()) {
    ...
    const applicability = await currentStoreAnswer(localRecordId, {
      ...
      scope: request.payload?.scope ?? STORE_SCOPES.BOARD_SQUARE_V1,
    });
