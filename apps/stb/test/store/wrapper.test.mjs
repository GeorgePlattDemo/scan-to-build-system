...
test('USER_DEFINED_BOARD_V1 re-evaluates every formal Store request and preserves a fresh receipt', async (t) => {
  ...
  assert.deepEqual(
    definition.parts.map((part) => [part.partId, part.lengthIn, part.features[0].xIn]),
    [['PART-1', 16, 8], ['PART-2', 16, 8]],
  );
  ...
  assert.equal(travel.parts[0].features[0].kind, 'SPOT_ON_LOCATION');
  ...
  assert.equal(body.rawEstimate.travel.finalRemainderIn, 27.625);
  assert.equal(body.rawEstimate.engine.version, '0.3.0');
...
test('USER_DEFINED_BOARD_V1 18-in same-span demand preserves definition and receives Store 72', async (t) => {
  const { adapter } = await withHost(t);
