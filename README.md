...
<a href="https://github.com/GeorgePlattDemo/scan-to-build-system/blob/preview/current-ui/README.md"><kbd>▶ OPEN CURRENT VISUAL CHECKPOINT</kbd></a>
&nbsp;
<a href="http://localhost:4317/"><kbd>▶ OPEN CURRENT APP — LOCAL</kbd></a>

- **Current visual checkpoint** is generated by Playwright from the actual `apps/stb/` application source. The checkpoint names the exact source branch and commit. If those do not match the work you intend to review, do not treat the pictures as current.
- **Current app — local** opens the actual application landing page at its fixed origin from the repository branch you have checked out. Start the local server first:
...
The former `scan-to-build-review/working-app.html` link is historical donor/evaluation material and is no longer the System front-door preview.
